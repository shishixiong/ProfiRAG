...
| 问题类型 | 推荐工具 | 原因 |
|---------|---------|------|
| 简单概念问题 | retrieve_and_answer | 一步完成，效率高 |
...
4. **评估结果**：
   ...
   - 结果与问题不相关：换用其他工具重新检索
5. **判断终止**：
   - 已获得足够信息 → 调用 generate_answer
   - 尝试3种工具仍无结果 → 调用 generate_answer（说明信息不足）
   - 已达 max_iterations → 调用 generate_answer（说明超时）
6. **生成回答**：调用 generate_answer，基于检索结果生成最终回答
